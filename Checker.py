...
# --- 4. قائمة النقاط (205 نقطة) ---
# سيتم ترتيبها أوتوماتيكياً بواسطة الكود لتجنب الخطوط القاطعة
RAW_POINTS = [
...
# --- 5. دالة الإصلاح الأوتوماتيكي (ترتيب النقاط دائرياً) ---
def sort_points_clockwise(points):
    """
    تقوم هذه الدالة بحساب مركز البلدة، ثم ترتيب جميع النقاط
    بناءً على الزاوية حول المركز، مما يزيل الخطوط القاطعة نهائياً.
    """
    # 1. حساب المركز (Centroid)
    center_lat = sum(p[0] for p in points) / len(points)
    center_lon = sum(p[1] for p in points) / len(points)

    # 2. الترتيب حسب الزاوية (atan2)
    def get_angle(point):
        return math.atan2(point[1] - center_lon, point[0] - center_lat)

    sorted_points = sorted(points, key=get_angle)
    
    # 3. التأكد من إغلاق الشكل
...
# تطبيق الترتيب فوراً
BOUNDARY_POINTS = sort_points_clockwise(RAW_POINTS)
