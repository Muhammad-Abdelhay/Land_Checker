...
def build_map(lat: float, lon: float, is_inside: bool):
    ...
    for r, op in [(28, 0.07), (16, 0.14), (8, 0.38)]:
        folium.CircleMarker(
            location=[lat, lon],
            radius=r,
            color=pin_color,
...
with col_side:
    ...
    # الفاصل
    st.markdown(
        '<div class="or-div">أو أدخل يدوياً</div>', unsafe_allow_html=True
    )
    ...
    with st.form("coord_form", clear_on_submit=False):
        ...
        submitted = st.form_submit_button(
            "🔍  بدء الفحص", use_container_width=True
        )
    ...
    if submitted:
        if not user_input.strip():
            ...
            if parsed:
                ...
                # ── لوحة النتيجة ──
                st.markdown(
                    '<div class="result-panel">', unsafe_allow_html=True
                )
